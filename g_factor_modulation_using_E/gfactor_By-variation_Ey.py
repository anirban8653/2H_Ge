...
def make_system(Ny, Nz, L, kx, Bf, Ey):
    a = L / (Ny + 1)
    syst = kwant.Builder()
...
# ---------------------------------------------
def get_top_vb_gap(Ny, Nz, L, kx, Bz, Ey, smatrixfull):
    syst = make_system(Ny, Nz, L, kx, Bz, Ey)
...
np.savetxt(
    f"gfactor_yy_vs_Ey_N{N}_L{size_list[0]}.dat",
    results,
