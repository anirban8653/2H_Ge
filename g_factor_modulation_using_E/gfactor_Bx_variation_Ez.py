...
# ---------------------------------------------
def get_top_vb_gap(Bf, L, Ez, smatrixfull):
    syst = make_system(Bf, L, Ez)
...
np.savetxt(
    f"gfactor_xx_vs_Ez_N{N}_L{size_list[0]}.dat",
    results,
    header="size(nm)  Ez(eV/Angstrom)  Ez(V/um)  Bx(T)  gap_plus(eV)  gap_minus(eV)  gap_avg(eV)  g_xx"
)