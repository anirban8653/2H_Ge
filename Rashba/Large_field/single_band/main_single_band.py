...
emax_v_um = 3.0
Ef_values = np.array([emax_v_um])*1e-4#np.arange(0.00000, 0.000012, 0.000003)
sigma_val = 0.12

print(f"running for N grid-size = {N}")
...
def process_kx(kx):
    ...
    num_eigenvectors = 2
    w, v = eigsh(H, k=num_eigenvectors, sigma=sigma_val)
    w = np.sort(w)
    eigval1, eigval2 = w[0], w[1]
    # gap = np.abs(eigval1 - eigval2)

    return kx, eigval1, eigval2
...
#     for Ef in Ef_values:
#         energylist = []
#         for idx, kx in enumerate(kpoints):
#             kx, eigval1, eigval2 = process_kx(kx)
#             energylist.append([kx,eigval1,eigval2])
#             print(idx, np.round(kx,5), np.round(eigval1,5), np.round(eigval2,5))
#         energylist = np.array(energylist)

#         # np.savetxt(f"band_data_y_E{(Ef * 1e4):.3f}_size{N}_nk{nk}.dat", energylist)
#         if flagy == 1:
#             np.savetxt(f"band_data_y_E{(Ef * 1e4):.3f}_size{L}_nk{nk}.dat", energylist)
#         if flagy == 0:  
#             np.savetxt(f"band_data_z_E{(Ef * 1e4):.3f}_size{L}_nk{nk}.dat", energylist)
